...
Поле над списком ищет сразу по нескольким полям: IP, хеш сети, `visitor_id`,
`session_id` и содержимому списков. Регистр не важен, работает частичное
совпадение — `кос` найдёт «Костя».
...
человека или всей его сети.

| Параметр | Что делает |
|---|---|
| `q` | свободный поиск по всем полям выше |
...
```bash
curl -u admin:ПАРОЛЬ https://wheel.rprokhorov.ru/api/sessions
curl -u admin:ПАРОЛЬ -G --data-urlencode 'q=Костя' https://wheel.rprokhorov.ru/api/sessions
curl -u admin:ПАРОЛЬ 'https://wheel.rprokhorov.ru/api/sessions?ip=195.238.73.62'
curl -u admin:ПАРОЛЬ 'https://wheel.rprokhorov.ru/api/sessions?session=<session_id>'
```
...
⚠️ **На время внутреннего тестирования собираются сами варианты из колеса**
(`items_text`) — чтобы разбирать пользовательский опыт. Не больше 30 позиций,
каждая обрезается до 60 символов.

Плашка в футере сайта об этом прямо говорит: «сохраняем статистику посещений
и варианты из колеса». **Если сайт откроется посторонним, сбор текстов нужно
выключить** — убрать `items_text` из `wofSetContext` в `js/analytics.js`
и вернуть плашке прежнюю формулировку.
...
Коллектор пропускает только эти ключи и только числовые значения: если клиент
пришлёт строку, она будет отброшена на входе.
