...
Фильтр по `visitor_id` показывает все визиты одного человека — так видно возвраты.

Эндпоинты:

```bash
curl -u admin:ПАРОЛЬ https://wheel.rprokhorov.ru/api/sessions
curl -u admin:ПАРОЛЬ 'https://wheel.rprokhorov.ru/api/sessions?visitor=<visitor_id>'
