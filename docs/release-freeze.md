...
1.6.3 — кандидат для заморозки функциональности после аудита безопасности.
Ветка подготовки: `security/freeze-1.6.3`. Публикация, тег и production-деплой —
отдельный шаг после проверки изменений владельцем.
...
docker compose run --rm --no-deps --user 0 --cap-add CHOWN \
  --entrypoint chown collector -R 1000:1000 /data
